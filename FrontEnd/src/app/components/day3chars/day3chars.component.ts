...
export class Day3charsComponent extends BaseCharacterClassComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._mainService.getDay3Characters(1).subscribe(data => this.Mondstadt = data);
    this._mainService.getDay3Characters(2).subscribe(data => this.Liyue = data);
    this._mainService.getDay3Characters(3).subscribe(data => this.Inazuma = data);
  }
